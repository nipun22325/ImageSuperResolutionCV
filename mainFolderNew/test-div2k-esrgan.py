...
model_path = f'/content/ImageSuperResolutionCV/mainFolderNew/models/{model}.pth'  # models/RRDB_ESRGAN_x4.pth OR models/RRDB_PSNR_x4.pth
device = torch.device('cuda')  # if you want to run on CPU, change 'cuda' -> cpu
# device = torch.device('cpu')

test_img_folder = '/content/ImageSuperResolutionCV/mainFolderNew/images/div2k/LRx4/DIV2K_valid_LR_bicubic/X4/*'
hr_img_folder = '/content/ImageSuperResolutionCV/mainFolderNew/images/div2k/HR/DIV2K_valid_HR'  # Path to high-resolution ground truth images
...
for path in glob.glob(test_img_folder):
    ...
    hr_base = base.replace('x4', '') if 'x4' in base else base
    ...
    # Save output image
    cv2.imwrite('/content/ImageSuperResolutionCV/mainFolderNew/results/{:s}_rlt.png'.format(base), output_uint8)
